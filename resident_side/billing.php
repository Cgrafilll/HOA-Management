...
try {
    ...
    try {
        ...
        // Only show pending and partial statuses
        ...
        // Search filter (invoice number only)
        if (!empty($searchQuery)) {
            $whereConditions[] = "md.invoice_number LIKE ?";
        ...
        if ($categoryFilter === 'all' || $categoryFilter === 'amenity') {
            ...
            // Only show pending and partial amenity bookings
            $amenityWhereConditions[] = "LOWER(ab.status) IN ('pending', 'partial')";
            
            // Search filter for amenity (invoice number only)
            if (!empty($searchQuery)) {
                $amenityWhereConditions[] = "ab.invoice_number LIKE ?";
                $searchParam = '%' . $searchQuery . '%';
        ...
        $invoices = [];
        $error_message = "Error fetching billing statements: " . $e->getMessage();
    }
...
<head>
    ...
    <title>NSSHAI HOA Management - My Billing</title>
            ...
            <div class="bg-white shadow rounded p-3">
                ...
                <div class="p-3">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="fw-semibold">My Billing Statements</div>
                    </div>
                            ...
                            <div class="border rounded-3" style="max-height: 600px; overflow-y: auto; scroll-behavior: smooth;">
                                <div class="list-group list-group-flush">
                                    <?php if (!empty($invoices)): ?>
                                        ...
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <div class="p-5 text-muted text-center">No billing statements found.</div>
                                    <?php endif; ?>
                                                </div>
                            </div>
                        </div>
                                            ...
                                            <div class="row mb-3">
                                                ...
                                                <div class="col-4">
                                                    <div class="text-end small">
                                                        <div class="fw-bold mb-2 fs-6">Billing No. <?= htmlspecialchars($selectedInvoice['invoice_number']); ?></div>
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-8">
                                                    ...
                                                    <div class="small">
                                                        <div class="mb-1"><span class="fw-semibold">Name:</span> <?= htmlspecialchars($selectedInvoice['full_name']); ?></div>
                                                        <div class="mb-1"><span class="fw-semibold">Household ID:</span> <?= htmlspecialchars($selectedInvoice['household_id']); ?></div>
                                                        <div><span class="fw-semibold">Billing Date:</span> <?= date('M d, Y', strtotime($selectedInvoice['created_at'])); ?></div>
                                                    </div>
                                                </div>
                                                <div class="col-4">
                                                    <div class="text-end small">
